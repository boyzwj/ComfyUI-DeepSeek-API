...
class DeepSeekAPIConfig:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "temperature": ("FLOAT", {"default": 1, "min": 0.0, "max": 1.0, "step": 0.1}),
                "max_tokens": ("INT", {"default": 512}),
                "stream": ("BOOLEAN", {"default": False, "label_on": "启用", "label_off": "关闭"}),
                "api_key": ("STRING", {
                    "default": "",
                    "multiline": True,  # 允许多行输入
                    "placeholder": "在此输入API密钥（可选，推荐填写在config文件中）"  
                }),
            },
            "hidden": {
    ...
    def load_config(self, config_name: str, temperature: float,  max_tokens: int, stream: bool, api_key: str, unique_id=None):
        # 如果之前加载失败，尝试重新加载一次配置
        ...
        if not self._config_cache:
            raise ValueError("没有可用的配置，请检查 config.json文件")
        ...
        # 优先使用配置文件中的API密钥
        final_api_key = model_config.get("api_key") or api_key.strip()
        if not final_api_key:
            raise ValueError("API密钥未配置，请在配置文件或节点参数中提供有效的API密钥")
        
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {final_api_key}"  # 使用动态插入
        }
